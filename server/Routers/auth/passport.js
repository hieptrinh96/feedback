    ...
    new GoogleStrategy(
      {
        ...
        callbackURL: "http://localhost:5000/auth/google/callback",
          //This means after signin on what route google should redirect
        ...
        db.query(
          ...
          (err, user) => {
            ...
            if (!err && user.length != 0) {  // checking whether user exist or not
              ...
              db.query(
                "insert into users set userName = ?, userEmail = ?",
...
router.get("/google/callback", passport.authenticate("google",), (req, res) => {
    ...
    if (req.user){
      ...
      const googleAuthToken = jwt.sign({googleAuthToken: req.user[0].email}, "xe6rctyuvi", {expiresIn:86400000 })
  ...
  router.get("/login/success", (req, res) => {
    if (req.user) {
      res.status(200).json({
        success: true,
        message: "successful",
        user: [req.user[0].userName, req.user[0].userEmail]
      });
    }
  });
